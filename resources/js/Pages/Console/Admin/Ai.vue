    ...
    <ConsoleLayout title="AI Settings">
        <div class="px-4 sm:px-6 lg:px-8 py-8 max-w-5xl mx-auto">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <!-- Left: AI Providers (unified card) -->
                <div class="lg:col-span-2">
                    <div class="tl-card tl-card--flush">
                        <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-800">
                            <h2 class="text-sm font-medium text-slate-300">AI Providers</h2>
                            <span class="text-xs text-slate-500">Lowest priority tried first</span>
                        </div>

                        <!-- Empty state -->
                        <div v-if="localProviders.length === 0" class="py-8 text-center px-5">
                            <TlIcon name="sparkles" class="w-7 h-7 mx-auto mb-2 text-slate-600" />
                            <p class="text-sm text-slate-400">No providers configured yet.</p>
                            <p class="text-xs text-slate-500 mt-0.5">Add one below to enable <strong class="text-slate-300">--summarize</strong> and <strong class="text-slate-300">--handoff</strong>.</p>
                        </div>
                        ...
                        <!-- Add provider form (compact inline) -->
                        <div class="border-t border-slate-800 px-4 py-4 mt-3">
                ...
                <div class="space-y-5">
                    ...
                    <div class="tl-card">
                        ...
                        <p class="text-xs text-slate-500 mb-4">Generate a token so <code class="text-indigo-400">ticketlens sync</code> can pull your connections to any machine. Shown once — store it securely.</p>
                        ...
                        <div v-if="newToken" class="mb-4 rounded-lg bg-green-950/40 border border-green-700/40 px-3 py-3">
                            ...
                            <p class="text-xs text-slate-500 mt-2">Run <code class="text-indigo-400">ticketlens login</code> to connect.</p>
                        </div>

                        <!-- Existing token -->
                        <div v-if="cli_token && !newToken" class="flex items-center justify-between mb-4">
                            <div>
                                <p class="text-sm text-slate-300 font-medium">{{ cli_token.name }}</p>
                                <p class="text-xs text-slate-500 mt-0.5">
                                    Created {{ formatDate(cli_token.created_at) }}
                                    <template v-if="cli_token.last_used_at"><br>Last used {{ formatDate(cli_token.last_used_at) }}</template>
                                </p>
                            </div>
                            <span class="text-xs px-2 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/25 shrink-0 ml-2">Active</span>
                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button v-if="cli_token" @click="revokeToken" class="tl-btn tl-btn--ghost text-xs text-red-400 hover:text-red-300">
                                Revoke
                            </button>
                        </div>
                    </div>
